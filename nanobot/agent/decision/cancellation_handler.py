...
from .models import CancellationRequest, DecisionRequest, DecisionResult
...
class CancellationHandler:
    ...
    async def handle_request(self, request: DecisionRequest) -> DecisionResult:
        ...
        try:
            ...
            # 处理取消请求
            action, action_data = await self._handle_cancellation(
                cancellation_request, request.task
            )
    ...
    async def _handle_cancellation(
        self, cancellation: CancellationRequest, task: Optional[Task]
    ) -> (str, Dict[str, Any]):
        """
        处理取消请求
    ...
    async def _handle_task_cancellation(
        self, cancellation: CancellationRequest, task: Task
    ) -> (str, Dict[str, Any]):
        """
        处理任务取消请求
        ...
        if task.status in ["completed", "failed", "cancelled"]:
            return "task_already_ended", {"task_id": task.id, "status": task.status}
    ...
    async def _handle_task_id_cancellation(
        ...
        if not task:
            return "task_not_found", {"task_id": cancellation.task_id}

        return await self._handle_task_cancellation(cancellation, task)

    async def _handle_active_tasks_cancellation(
        self, cancellation: CancellationRequest, active_tasks: list
    ) -> (str, Dict[str, Any]):
        """
        处理正在运行的任务取消请求
