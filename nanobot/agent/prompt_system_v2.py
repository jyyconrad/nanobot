...
class PromptSystemV2:
    ...
    def __init__(
        ...
        logger.info(f"PromptSystemV2 initialized with config_path={self.config_path}, workspace={self.workspace}")
        ...
        return {
            ...
            "layers": {
                ...
                "workspace": {
                    ...
                    "load_order": 2,
                    "files": [
                        {"path": "workspace/agents.md", "section": "agents", "allow_override": True},
                        {"path": "workspace/practices.md", "section": "practices", "allow_override": True}
                    ]
                },
                "user": {
                    "required": True,
                    "load_order": 3,
                    "files": [
                        {"path": "user/profile.md", "section": "user_profile", "allow_override": True},
                        {"path": "user/preferences.md", "section": "preferences", "allow_override": True}
                    ]
                },
                "memory": {
                    "required": False,
                    ...
                    "files": [
                        ...
                        {"source": "workspace", "file": "MEMORY.md", "section": "user_memory"}
                    ]
                },
                "decisions": {
                    "required": False,
                    ...
                    "files": [
                        {"path": "decisions/task_analysis.md", "key": "task_analysis_prompt"},
                        {"path": "decisions/skill_selection.md", "key": "skill_selection_prompt"},
                        {"path": "decisions/agent_selection.md", "key": "agent_selection_prompt"}
                    ]
                }
            },
            "templates": {
    ...
    def _register_default_hooks(self) -> None:
        ...
        # MainAgent 提示词构建完成
        self.register_hook("on_main_agent_prompt_built", self.on_main_agent_prompt_built)
        # Subagent 提示词构建完成
    ...
    def on_main_agent_prompt_built(self, **kwargs) -> None:
        ...
        logger.info(f"PromptSystemV2 MainAgent prompt built: {len(prompt)} chars, {len(sections)} sections")
    ...
    def load_prompts(self, paths: Optional[List[str]] = None) -> None:
        ...
        for layer_name, layer_config in sorted(layers.items(), key=lambda item: item[1].get("load_order", 999)):
            # 检查条件是否满足
            ...
            # 触发层加载完成钩子
            self.trigger("on_layer_loaded", layer_name=layer_name, content=layer_content)
    ...
    def _load_workspace_override(self, path: str) -> Optional[str]:
        ...
        try:
            ...
            logger.error(f"Failed to load workspace override file {workspace_file}: {e}")
            return None
        ...
        **kwargs
        ...
        for layer_name in ["core", "workspace", "user"]:
            if layer_name in self.config.get("layers", {}):
                core_sections.update(self._load_layer(layer_name, self.config["layers"][layer_name]))
    ...
    def get_stats(self) -> Dict:
        ...
        return {
            ...
            "hooks_registered": {name: len(callbacks) for name, callbacks in self.hooks.items()},
...
def get_prompt_system_v2(
    ...
    if _prompt_system_v2_instance is None:
        _prompt_system_v2_instance = PromptSystemV2(config_path=config_path, workspace=workspace)
