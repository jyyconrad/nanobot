...
try:
    ...
    logger.warning("Matrix dependencies not installed. Run: pip install nanobot-ai[matrix]")
...
class MatrixChannel(BaseChannel):
    ...
    def __init__(self, config: MatrixConfig, bus: MessageBus):
        ...
        self._data_dir = Path.home() / ".nanobot" / "matrix-store"

    async def start(self) -> None:
        if not MATRIX_AVAILABLE:
            logger.error("Matrix SDK not installed. Run: pip install nanobot-ai[matrix]")
            return
    ...
    def _media_dir(self) -> Path:
        d = Path.home() / ".nanobot" / "media" / "matrix"
