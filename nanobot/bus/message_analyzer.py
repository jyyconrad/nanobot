...
class MessageAnalyzer:
    ...
    def analyze_message(
        ...
        active_tasks: Optional[List[Task]] = None
    ) -> AnalysisResult:
        """
        分析消息与现有任务的关联
        ...
        logger.debug(f"Analyzing message: {message.content[:50]}... "
                   f"against {len(active_tasks)} active tasks")
        ...
        logger.debug(f"Best matching task: {best_match.id} "
        ...
        if best_score > 0.6:
            ...
            if self._contains_correction_keywords(message.content):
                ...
                logger.debug("Medium similarity but no correction keywords, "
                           "suggesting create_task")
                return AnalysisResult(
                    action=AnalysisAction.CREATE_TASK,
                    target_task_id=None,
