...
async def test_context_compression_time():
    ...
    medium_conversation = []
    for i in range(20):
        medium_conversation.append({"role": "user", f"content": f"问题 {i}"})
        medium_conversation.append({"role": "assistant", f"content": f"回答 {i}"})
    ...
    large_conversation = []
    for i in range(100):
        large_conversation.append({"role": "user", f"content": f"问题 {i}"})
        large_conversation.append({"role": "assistant", f"content": f"回答 {i}"})
...
async def test_context_compression_ratio():
    ...
    conversation = []
    for i in range(50):
        conversation.append({"role": "user", f"content": f"问题 {i}"})
        conversation.append({"role": "assistant", f"content": f"回答 {i}"})
