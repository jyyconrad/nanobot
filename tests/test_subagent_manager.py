...
async def test_spawn_subagent(mock_agno_subagent, subagent_manager, mock_subagent_task):
    ...
    mock_instance = Mock()
    mock_instance.execute = AsyncMock(return_value=SubagentResult(
...
async def test_get_subagent_status(mock_agno_subagent, subagent_manager, mock_subagent_task):
    ...
    assert status.status in [SubagentStatus.ASSIGNED, SubagentStatus.RUNNING, SubagentStatus.COMPLETED]
...
@pytest.mark.asyncio
@patch('nanobot.agent.subagent.manager.AgnoSubagent')
async def test_register_and_unregister_callback(mock_agno_subagent, subagent_manager, mock_subagent_task):
    """测试注册和取消注册回调"""
    # 设置 mock，让它立即完成
    mock_instance = Mock()
    mock_instance.execute = AsyncMock(return_value=SubagentResult(
