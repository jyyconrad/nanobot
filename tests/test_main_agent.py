...
from nanobot.agent.subagent.models import SubagentTask, SubagentResult, SubagentState, SubagentStatus
...
def mock_task_planner():
    """创建模拟的 TaskPlanner"""
    mock = Mock()
    from nanobot.agent.planner.models import TaskType, TaskPriority
    mock.plan_task = AsyncMock(return_value=TaskPlan(
...
def mock_decision_maker():
    ...
    mock = Mock()
    mock.make_decision = AsyncMock(return_value=DecisionResult(
        success=True,
        action="reply",
        data={},
...
async def test_process_message_new_task(
    ...
    mock_tp = Mock()
    from nanobot.agent.planner.models import TaskType, TaskPriority
    mock_tp.plan_task = AsyncMock(return_value=TaskPlan(
        task_type=TaskType.CODE_GENERATION,
        priority=TaskPriority.MEDIUM,
        complexity=0.7,
        steps=["step1", "step2"],
    ...
    mock_dm = Mock()
    mock_dm.make_decision = AsyncMock(return_value=DecisionResult(
        success=True,
        action="reply",
        data={},
    ...
    mock_hooks = Mock()
    for attr in ['on_message_receive', 'before_planning', 'after_planning', 
                'before_decision', 'after_decision', 'on_response_send']:
        setattr(mock_hooks, attr, AsyncMock())
    setattr(mock_hooks, 'on_message_receive', AsyncMock(return_value=Mock(allow=True, block=False, modified_message=None)))
    setattr(mock_hooks, 'before_planning', AsyncMock(return_value=Mock(allow=True, block=False, modified_message=None)))
    setattr(mock_hooks, 'before_decision', AsyncMock(return_value=Mock(allow=True, block=False, modified_message=None)))
...
async def test_process_message_correction(
    ...
    mock_dm.make_decision = AsyncMock(return_value=DecisionResult(
        ...
        action="reply",
        data={},
        message="Corrected task completed"
    ))
    mock_dm_cls.return_value = mock_dm
